(function (nx) {
    ...
    (function getComputedStyleForIE(){
    ...
    function _fixEvent(e){
        var _isIE = nx.Env.browser().name.toLowerCase()==='ie';
        e = e||window.event;
    ...
    var Element = nx.define('nx.dom.Element', nx.dom.Node, {
        methods: {
            ...
            addEventListener: function (name, listener, useCapture) {
                if(nx.Env.browser().name.toLowerCase()=='firefox'&&name=='mousewheel'){ name = 'DOMMouseScroll'; }
                if(useCapture==undefined){ useCapture = false; }
                if(this.$dom.attachEvent){
                    this.$dom.attachEvent('on'+name, function(e){ listener(_fixEvent(e)); }, useCapture);
                }else {
                    this.$dom.addEventListener(name, function(e){ listener(_fixEvent(e)); }, useCapture);
                }
            },
            /**
            ...
            removeEventListener: function (name, listener, useCapture) {
                if(useCapture==undefined){ useCapture = false; }
                if(this.$dom.detachEvent){
                    this.$dom.detachEvent('on'+name, function(e){ listener(_fixEvent(e)); }, useCapture);
                }else {
                    this.$dom.removeEventListener(name, function(e){ listener(_fixEvent(e)); }, useCapture);
                }
